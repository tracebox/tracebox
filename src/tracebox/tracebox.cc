...
int doTracebox(Packet *pkt, tracebox_cb_t *callback, string& err, void *ctx)
{
	...
	for (int ttl = 1; ttl <= hops_max; ++ttl) {
		...
		else{ // Writie both pkt & rcv to pcap file
...
int main(int argc, char *argv[])
{
	...
	ShowWarnings = 0;
	while ((c = getopt(argc, argv, "l:i:m:s:p:d:c:f:hnv6uwjq")) != -1) {
		switch (c) {
			case 'i':
				iface = optarg;
				break;
...
"  -c server_url               Specify a server where captured packets will be sent (upload pcap format file).\n"
